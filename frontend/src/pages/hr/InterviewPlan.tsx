...
import { ShieldCheck, Info, Sparkles, AlertTriangle, Loader2 } from "lucide-react";
...
export default function InterviewPlan() {
  const { generateInterviewPlan, interviewPlan, syncState } = useWorkflow();
  const [isGenerating, setIsGenerating] = useState(false);
  const visibleGroups = interviewPlan?.questions.length
    ? [{
      title: "Backend-generated from current evidence map",
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              ...
              {interviewPlan?.objective ? ` Objective: ${interviewPlan.objective}` : ''}
            </p>
          </div>
          <Button variant="outline" className="gap-2" onClick={handleGenerate} disabled={isGenerating}>
            {isGenerating ? <Loader2 className="h-4 w-4 animate-spin text-teal-700" /> : <Sparkles className="h-4 w-4 text-teal-700" />}
