...
def provider_status(settings: Settings, model: ModelProvider, exa: ExaClient, google: GoogleClients) -> dict[str, Any]:
    ...
    return {
        ...
        "database": {
            "provider": "supabase_postgres" if settings.database_url or settings.supabase_url else "fixture",
            "configured": bool(settings.database_url or settings.supabase_url),
            "status": "ready" if supabase_ready or settings.database_url else "fixture",
        },
        "auth": {
